...
$(document).ready(function () {
  ...
  $('.select2').select2();
  var bannerImageElement = $('#bannerImage');
  if (bannerImageElement.length) {
    bannerImageElement.addClass("loaded");
  }
  var bannerElement = $('.banner-wrap--title');
  if (bannerElement.length) {
    var bannerText = bannerElement.html().split(" ");
    if (bannerText.length >= 4) {
      bannerText[3] = "<span><strong>".concat(bannerText[3], "</strong></span>");
    }
  ...
  $('.section-title').each(function () {
    ...
    if (sectionTitle.length >= 2) {
      sectionTitle[sectionTitle.length - 2] = "<strong>".concat(sectionTitle[sectionTitle.length - 2], "</strong>");
  ...
  $.each(params, function (key, values) {
    if (Array.isArray(values)) {
      values.forEach(function (value) {
        var keyName = value.replace('_', ' ');
        var searchKey = keyName.split(' ').join(' ');
  ...
  $('.team-slider').slick(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty({
    dots: true,
    infinite: false,
    speed: 300,
    centerMode: true
  }, "infinite", true), "autoplay", true), "slidesToShow", 3), "autoplaySpeed", 2000), "speed", 2000), "slidesToScroll", 1), "centerpadding", '10px'), "responsive", [{
    breakpoint: 1024,
    settings: {
      slidesToShow: 1,
      slidesToScroll: 1,
